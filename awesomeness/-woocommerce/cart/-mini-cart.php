			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<li class="<?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						$product_ids = array ("2144", "2171", "Irix", "Linux");
						if (in_array($product_id, $product_ids)) 
						//if ($product_id == "2144") //2144 is het product id van het bestel formulier
						{
						$item_data = array();		
										
						$item_data = apply_filters( 'woocommerce_get_item_data', $item_data, $cart_item );
						//print_r($item_data);
						if ( isset($item_data['3']['value'])) {
  						echo '<img src="'.$item_data['3']['value'].'" class="filter" data-filter="'.$item_data['2']['value'].'" width="180" />';
						}
						}
						else
						{
